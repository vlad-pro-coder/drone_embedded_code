...
VL53L0XDriver::VL53L0XDriver(int addr = 0x29) : address(addr), fd(-1)
{
...
uint16_t VL53L0XDriver::readDistance()
{
    if (!writeReg(0x00, 0x01))
        return 0;
    int timeout = 1000;
    while (timeout-- > 0)
    {
        if (readReg(0x13) & 0x07)
            break;
        usleep(1000);
    }
    if (timeout <= 0)
...
void VL53L0XDriver::logDistance(uint16_t distance)
{
...
static void VL53L0XDriver::signalHandler(int sig)
{
    running = false;
...
bool VL53L0XDriver::initializeSensor()
{
    usleep(30000);
    if (readReg(0xC0) != 0xEE)
        return false;
    if (!writeReg(0x88, 0x00))
        return false;
    if (!writeReg(0x80, 0x01))
        return false;
    if (!writeReg(0xFF, 0x01))
        return false;
    if (!writeReg(0x00, 0x00))
        return false;
    if (!writeReg(0x91, 0x3C))
        return false;
    if (!writeReg(0x00, 0x01))
        return false;
    if (!writeReg(0xFF, 0x00))
        return false;
    if (!writeReg(0x80, 0x00))
        return false;
    if (!writeReg(0x60, 0x00))
        return false;
    usleep(2000);
    return true;
}

bool VL53L0XDriver::writeReg(uint8_t reg, uint8_t value)
{
    buffer[0] = reg;
    buffer[1] = value;
